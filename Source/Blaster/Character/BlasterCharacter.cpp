...
void ABlasterCharacter::PlayFireMontage(bool bAiming)
{
	...
	if (AnimIsntance && FireWeaponMontage)
	{
		AnimIsntance->Montage_Play(FireWeaponMontage);
		FName SectionName;
		SectionName = bAiming ? FName("RifleAim") : FName("RifleHip");
		AnimIsntance->Montage_JumpToSection(SectionName);
	}
}
...
void ABlasterCharacter::PlayReloadMontage()
{
	...
	UAnimInstance* AnimIsntance = GetMesh()->GetAnimInstance();
	if (AnimIsntance && ReloadMontage)
	{
		AnimIsntance->Montage_Play(ReloadMontage);
		FName SectionName;
...
void ABlasterCharacter::PlayHitReactMontage()
{
	...
	UAnimInstance* AnimIsntance = GetMesh()->GetAnimInstance();
	if (AnimIsntance && HitReactMontage)
	{
		AnimIsntance->Montage_Play(HitReactMontage);
...
void ABlasterCharacter::Elim()
{
	...
	GetWorldTimerManager().SetTimer(
		ElimTimer,
		this,
		&ABlasterCharacter::ElimTimerFinished,			// 时间结束后调用重生
		ElimDelay
	);
}
...
void ABlasterCharacter::MulticastElim_Implementation()
{
	...
	GetCharacterMovement()->DisableMovement();				// 禁用WASD
	GetCharacterMovement()->StopMovementImmediately();		// 警用转向
	if (BlasterPlayerController)
	{
		DisableInput(BlasterPlayerController);				// 禁用输入
	}
	// DIsable collision
...
void ABlasterCharacter::HideCameraIfCharacterClose()
{
	...
	// 判断摄像机与角色的距离是否小于阈值，如果是，则隐藏角色和武器
...
void ABlasterCharacter::SetOverlappingWeapon(AWeapon* Weapon)
{
	...
	// OnRep_OverlappingWeapon是在复制到客户端时才调用，在服务器端永远都调用不了，所以增加一个判断，用来显示服务器端的
