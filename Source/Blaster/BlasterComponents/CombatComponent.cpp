...
void UCombatComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(UCombatComponent, CarriedAmmo, COND_OwnerOnly);		// 携带子弹只与自己有关
...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	EquippedWeapon->SetHUDAmmo();			// 捡枪之后更新显示
...
void UCombatComponent::UpdateShotgunAmmoValues()
{
	...
	bCanFire = true;			// 装弹过程中，弹夹有子弹了，就可以射击
...
void UCombatComponent::OnRep_EquippedWeapon()
{
	...
	{
		// 加在这是为了防止AttachActor在EquippedWeapon复制之前就调用了
		AttachActorToRightHand(EquippedWeapon);
		// 持枪之后，使用controller的旋转，不使用自己的旋转
		Character->GetCharacterMovement()->bOrientRotationToMovement = false;
		Character->bUseControllerRotationYaw = true;
...
void UCombatComponent::StartFireTimer()
{
	...
	Character->GetWorldTimerManager().SetTimer(
		FireTimer,
		this,
		&UCombatComponent::FireTimerFinished,		// 定时到达后执行的函数
		EquippedWeapon->FireDelay					// 延迟时间
	);
}
...
void UCombatComponent::PickupAmmo(EWeaponType WeaponType, int32 AmmoAmount)
{
	if (CarriedAmmoMap.Contains(WeaponType))
	{
		...
		UpdateCarriedAmmo();				// 更新CarriedAmmo和HUD
	}
